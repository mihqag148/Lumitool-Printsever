...
$LogPath = Join-Path $env:TEMP "Lumitool_Printsever_Setup_V4_8.log"
...
try {
    ...
    function Invoke-PrintUiAttribute {
        ...
        $safeName = ([string]$PrinterName).Replace('"','\"')
        $argLine =
            'printui.dll,PrintUIEntry /Xs /n "' +
            $safeName +
            '" attributes ' +
            $Attribute
        ...
        if (-not $p.HasExited) {
            try { $p.Kill() } catch {}
            throw ("PrintUIEntry " + $Attribute + " timeout sau 6 giây; đã dừng để app không bị treo.")
        }
    ...
    function Configure-RawCompletionMode {
        ...
        UiLog "QUEUE: bỏ Set-Printer đồng bộ để tránh driver SP46 khóa UI; cấu hình attributes bằng PrintUI"
    ...
    function Pause-LumitoolJobWatcher {
        try {
            ...
            if ([string]$task.State -ne "Disabled") {
                ...
                UiLog "WATCHER V2: tạm dừng trong lúc cấu hình queue"
                return $true
            }
        } catch {
            UiLog("WATCHER V2: không cần tạm dừng - " + $_.Exception.Message)
        }
        return $false
    }
    ...
    function Resume-LumitoolJobWatcher {
        ...
        try {
            ...
            UiLog "WATCHER V2: đã chạy lại"
        } catch {
            UiLog("WATCHER V2 cảnh báo: không khởi động lại được - " + $_.Exception.Message)
        }
    }
    ...
    $form.Text = "Lumitool Printsever Setup V4.8"
    ...
    UiLog "Lumitool Printsever Setup V4.8"
    UiLog "Hỗ trợ mọi máy in có driver Windows đã được cài."
    UiLog "A=9101 · B=9102 · C=9103 · Fast Spool · chống treo · Watcher V2 auto-discovery"
