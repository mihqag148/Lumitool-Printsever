...
$LogPath = Join-Path $env:TEMP "Lumitool_Printsever_Setup_V5_0.log"
...
if (-not $isAdmin) {
    try {
        ...
        [System.Windows.Forms.MessageBox]::Show("Cần quyền Administrator để cài máy in và cấu hình Windows.","Lumitool Printsever","OK","Error") | Out-Null
    }
    exit
}
...
try {
    ...
    function Connect-IP {
        ...
        $script:CurrentID =
            [string]$dev.ID
        $script:CurrentUID =
            (([string]$dev.UID).ToUpper() -replace '[^0-9A-F]', '')
                        ...
                        if (
                            ...
                            if (
                                ...
                                if (
                                    ...
                                    $uid =
                                        (([string]$o.uid).ToUpper() -replace '[^0-9A-F]', '')

                                    if (-not $uid) {
                                        $uid =
                                            (([string]$o.mac).ToUpper() -replace '[^0-9A-F]', '')
                                    }
    ...
    function Show-ScanResults {
        ...
        if ($devices.Count -eq 1) {
            ...
            $script:CurrentID = ([string]$d.ID).ToUpper()
            $script:CurrentUID = (([string]$d.UID).ToUpper() -replace '[^0-9A-F]', '')
    ...
    function Disable-PortSnmp {
        ...
        try {
            ...
            UiLog "PORT/SNMP: đang tắt SNMP (timeout 6s)"
            ...
            $p = Start-Process -FilePath "cscript.exe" -ArgumentList $argLine -WindowStyle Hidden -PassThru -ErrorAction Stop
            ...
            if (-not $p.HasExited) {
                try { $p.Kill() } catch {}
                UiLog "PORT/SNMP: timeout; bỏ qua để không treo app"
                return
            }
            ...
            if ($p.ExitCode -eq 0) {
                ...
                UiLog ("PORT: tắt SNMP cảnh báo, exit=" + $p.ExitCode)
            }
        } catch {
    ...
    function Configure-RawCompletionMode {
        ...
        # V49_NO_FORCE_QUEUE_ATTRIBUTES
        # SP46 runtime testing showed that SetPrinter can fail and PrintUI /Xs
        # can block inside the vendor driver. Do not mutate Direct/Queued/BIDI here.
        # Keep the vendor queue usable and let Watcher V3 synchronize ESP DONE
        # back to the Windows print job.
        ...
        if (-not $check) {
            throw "Windows chưa đăng ký printer queue."
        }

        UiLog("QUEUE STATE: Direct=" + [string]$check.Direct + " Queued=" + [string]$check.Queued + " BIDI=" + [string]$check.EnableBIDI + " Keep=" + [string]$check.KeepPrintedJobs + " Offline=" + [string]$check.WorkOffline)
        UiLog "QUEUE MODE: giữ cấu hình driver Windows; không ép Direct/Queued để tránh treo SP46"
        UiLog "JOB COMPLETION: Watcher V3 sẽ đồng bộ ESP DONE -> Windows"
    }

    function Pause-LumitoolJobWatcher {
        try {
            $task = Get-ScheduledTask -TaskName "Lumitool Printsever Job Watcher" -ErrorAction Stop
            if ([string]$task.State -ne "Disabled") {
                ...
                UiLog "WATCHER V3: tạm dừng trong lúc cấu hình queue"
                return $true
            }
        } catch {
            UiLog("WATCHER V3: không cần tạm dừng - " + $_.Exception.Message)
        }
        return $false
    }

    function Resume-LumitoolJobWatcher {
        param([bool]$ShouldResume)
        if (-not $ShouldResume) { return }
        try {
            Start-ScheduledTask -TaskName "Lumitool Printsever Job Watcher" -ErrorAction Stop
            UiLog "WATCHER V3: đã chạy lại"
        } catch {
            UiLog("WATCHER V3 cảnh báo: không khởi động lại được - " + $_.Exception.Message)
        }
    }

    function Repair-TcpPortHost {
        param(
            [string]$PortName,
            [string]$IP,
            [int]$TcpPort
        ...
        if (-not (Test-Path $regPath)) {
            throw ("Không tìm thấy registry của TCP port " + $PortName)
        }

        UiLog("PORT SELF-HEAL: " + $PortName + " -> " + $IP + ":" + $TcpPort)
        $stopped = $false
        ...
        try {
            ...
            Set-ItemProperty -Path $regPath -Name "HostName" -Value $IP -ErrorAction Stop
            Set-ItemProperty -Path $regPath -Name "IPAddress" -Value $IP -ErrorAction Stop

            try {
                Set-ItemProperty -Path $regPath -Name "PortNumber" -Value ([int]$TcpPort) -ErrorAction SilentlyContinue
            } catch {}

            try {
                if (Get-ItemProperty -Path $regPath -Name "SNMP Enabled" -ErrorAction SilentlyContinue) {
                    Set-ItemProperty -Path $regPath -Name "SNMP Enabled" -Value 0 -ErrorAction SilentlyContinue
                }
            } catch {}
        } finally {
            if ($stopped) {
        ...
        $check = Get-PrinterPort -Name $PortName -ErrorAction SilentlyContinue

        if (
            -not $check -or
            [string]$check.PrinterHostAddress -ne [string]$IP
        ) {
            throw ("TCP port vẫn chưa cập nhật sang IP " + $IP)
        }

        UiLog "PORT SELF-HEAL: OK"
    }
    ...
    function Ensure-TcpPort {
        ...
        if ($existing) {
            $oldIp = [string]$existing.PrinterHostAddress

            if ($oldIp -ne [string]$IP) {
                UiLog("PORT: IP cũ " + $oldIp + " -> IP mới " + $IP)
                Repair-TcpPortHost -PortName $PortName -IP $IP -TcpPort $TcpPort
            } else {
                UiLog("PORT: đã tồn tại và IP đúng " + $PortName)
            }
    ...
    function Test-LumitoolJobWatcher {
        try {
            $task = Get-ScheduledTask -TaskName "Lumitool Printsever Job Watcher" -ErrorAction Stop
            $info = Get-ScheduledTaskInfo -TaskName "Lumitool Printsever Job Watcher" -ErrorAction Stop
            UiLog ("WATCHER V3: state=" + [string]$task.State + " lastResult=" + [string]$info.LastTaskResult)
            return $true
        } catch {
            UiLog ("WATCHER V3 cảnh báo: task chưa sẵn sàng - " + $_.Exception.Message)
            return $false
        }
    }

    function Wait-PrinterVisible {
        param(
            [string]$PrinterName,
            [string]$ExpectedPort,
        ...
        while ((Get-Date) -lt $deadline) {
            ...
            $w = Get-CimInstance -ClassName Win32_Printer -ErrorAction SilentlyContinue |
                Where-Object { [string]$_.Name -eq [string]$PrinterName } |
                Select-Object -First 1

            if ($p -and $w) {
                if ([string]$p.PortName -eq $ExpectedPort -and [string]$p.DriverName -eq $ExpectedDriver) {
                    return $true
                }
            }
    ...
    function Start-LumitoolOta {
        ...
        try {
            ...
            try {
                if ($suffix) {
                    $rawCancel = "admin:" + $suffix
                    $authCancel = [Convert]::ToBase64String([Text.Encoding]::ASCII.GetBytes($rawCancel))
                    $headersCancel = @{ Authorization = ("Basic " + $authCancel) }
                    Invoke-WebRequest -Uri ("http://" + $script:CurrentIP + "/support/cancel") -Method Post -Headers $headersCancel -UseBasicParsing -TimeoutSec 3 -ErrorAction SilentlyContinue | Out-Null
                    UiLog "OTA: đã yêu cầu ESP thoát maintenance sau lỗi"
                }
            } catch {}
