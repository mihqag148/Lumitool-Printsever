...
try {
    ...
    function Repair-TcpPortHost {
        ...
        try {
            ...
            try { Set-ItemProperty -Path $regPath -Name "PortNumber" -Value ([int]$TcpPort) -ErrorAction SilentlyContinue } catch {}
            try {
        ...
        if (-not $check -or [string]$check.PrinterHostAddress -ne [string]$IP) {
    ...
    exit 1
            ...
            $uid =
                (([string]$Device.UID).ToUpper() -replace '[^0-9A-F]', '')
