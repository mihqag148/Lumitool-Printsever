...
$LogPath = Join-Path $env:TEMP "Lumitool_Printsever_Setup_V4_6.log"
...
try {
    ...
    function Invoke-PrintUiAttribute {
        param(
            [string]$PrinterName,
            [string]$Attribute
        )
        ...
        if (-not (Test-Path $rundll32)) {
            throw "Không tìm thấy rundll32.exe của Windows."
        }
        ...
        $p = Start-Process -FilePath $rundll32 -ArgumentList $args -WindowStyle Hidden -Wait -PassThru -ErrorAction Stop
        UiLog("QUEUE/PRINTUI: " + $Attribute + " exit=" + [string]$p.ExitCode)

        if ($p.ExitCode -ne 0) {
            throw ("PrintUIEntry " + $Attribute + " thất bại, exit=" + $p.ExitCode)
        }
    }
    ...
    function Configure-RawCompletionMode {
        ...
        try {
            ...
            throw ("Set-Printer RAW/KeepPrintedJobs thất bại: " + $_.Exception.Message)
        }

        foreach ($attr in @("-direct","-queued","-enablebidi","-keepprintedjobs","-workoffline","-enabledevq")) {
            Invoke-PrintUiAttribute -PrinterName $PrinterName -Attribute $attr
        }

        $check = $null
        for ($i = 0; $i -lt 20; $i++) {
            Start-Sleep -Milliseconds 250
            $check = Get-CimInstance -ClassName Win32_Printer -ErrorAction SilentlyContinue | Where-Object { [string]$_.Name -eq [string]$PrinterName } | Select-Object -First 1

            if ($check -and (-not [bool]$check.Direct) -and (-not [bool]$check.Queued) -and (-not [bool]$check.EnableBIDI) -and (-not [bool]$check.KeepPrintedJobs) -and (-not [bool]$check.WorkOffline)) {
                break
            }
        }

        if (-not $check) {
            throw "Windows chưa đăng ký printer queue sau khi cấu hình."
        }

        UiLog("QUEUE VERIFY: Direct=" + [string]$check.Direct + " Queued=" + [string]$check.Queued + " BIDI=" + [string]$check.EnableBIDI + " Keep=" + [string]$check.KeepPrintedJobs + " Offline=" + [string]$check.WorkOffline)

        if ([bool]$check.Direct -or [bool]$check.Queued -or [bool]$check.EnableBIDI -or [bool]$check.KeepPrintedJobs -or [bool]$check.WorkOffline) {
            throw "Windows chưa áp dụng đúng Fast Spool Mode qua PrintUIEntry."
        }

        UiLog "QUEUE VERIFY: Spooling ON + Print while spooling + Direct OFF"
    }

    function Remove-LumitoolTargetQueue {
        param(
            [string]$PrinterName,
            [string]$PortName
        )

        $targets = @(Get-Printer -ErrorAction SilentlyContinue | Where-Object { ([string]$_.Name -eq [string]$PrinterName) -or ([string]$_.PortName -eq [string]$PortName) })

        foreach ($p in $targets) {
            $pn = [string]$p.Name
            UiLog("CLEAN: xóa queue cũ " + $pn)
            try { Get-PrintJob -PrinterName $pn -ErrorAction SilentlyContinue | Remove-PrintJob -Confirm:$false -ErrorAction SilentlyContinue } catch {}
            Remove-Printer -Name $pn -Confirm:$false -ErrorAction Stop
        }
        ...
        if ($targets.Count -gt 0) {
            ...
            do {
                Start-Sleep -Milliseconds 250
                $stillThere = @(Get-Printer -ErrorAction SilentlyContinue | Where-Object { ([string]$_.Name -eq [string]$PrinterName) -or ([string]$_.PortName -eq [string]$PortName) })
            } while ($stillThere.Count -gt 0 -and (Get-Date) -lt $deadline)

            if ($stillThere.Count -gt 0) {
                throw "Windows chưa xóa xong queue Lumitool cũ."
            }
        }

        $oldPort = Get-PrinterPort -Name $PortName -ErrorAction SilentlyContinue
        if ($oldPort) {
            UiLog("CLEAN: xóa TCP/IP port cũ " + $PortName)
            Remove-PrinterPort -Name $PortName -ErrorAction Stop
        }

        UiLog "CLEAN: slot Lumitool đã sạch; giữ nguyên driver hãng"
    }
    ...
    function Test-LumitoolJobWatcher {
        try {
            ...
            UiLog ("WATCHER V2: state=" + [string]$task.State + " lastResult=" + [string]$info.LastTaskResult)
            return $true
        } catch {
            UiLog ("WATCHER V2 cảnh báo: task chưa sẵn sàng - " + $_.Exception.Message)
            return $false
        }
    }
    ...
    function Install-PrinterFor {
        ...
        try {
            ...
            UiLog "CLEAN: dọn queue/port Lumitool cũ của slot trước khi cài"
            Remove-LumitoolTargetQueue -PrinterName $name -PortName $portName

            UiLog("BƯỚC 1/2: tạo TCP/IP port mới")
            ...
            UiLog("BƯỚC 2/2: tạo mới Windows printer queue")

            Add-Printer -Name $name -DriverName $driver -PortName $portName -Datatype "RAW" -ErrorAction Stop
            UiLog "QUEUE: tạo mới hoàn toàn, không kế thừa Direct/Queued của bản cũ"
            ...
            UiLog "Cấu hình RAW queue: Spooling ON, SNMP/BIDI OFF"
            ...
            UiLog "Kiểm tra Job Watcher V2 (đã được installer cài một lần)"
    ...
    $form.Text = "Lumitool Printsever Setup V4.6"
    ...
    UiLog "Lumitool Printsever Setup V4.6"
    UiLog "Hỗ trợ mọi máy in có driver Windows đã được cài."
    UiLog "A=9101 · B=9102 · C=9103 · Clean queue · Fast Spool · Watcher V2 auto-discovery"
