...
$LogPath = Join-Path $env:TEMP "Lumitool_Printsever_Setup_V4_5.log"
...
try {
    ...
    function Ensure-NativePrinterApi {
        if ("Lumitool.NativePrinter" -as [type]) {
            return
        }
...
namespace Lumitool
{
    public static class NativePrinter
    {
        ...
        private const UInt32 PRINTER_ACCESS_ADMINISTER = 0x00000004u;
        private const UInt32 PRINTER_ACCESS_USE        = 0x00000008u;
        ...
        public static UInt32 SetFastSpoolMode(string printerName)
        {
            ...
            // SetPrinter requires PRINTER_ACCESS_ADMINISTER. V4.4 opened the
            // handle with pDefault=NULL, which only gives the default access
            // and caused SetPrinter to fail even though the app was elevated.
            defaults.DesiredAccess = PRINTER_ACCESS_ADMINISTER | PRINTER_ACCESS_USE;

            if (!OpenPrinter(printerName, out hPrinter, ref defaults))
                throw new Win32Exception(
                    Marshal.GetLastWin32Error(),
                    "OpenPrinter(PRINTER_ACCESS_ADMINISTER) failed"
                );

            try
            {
                UInt32 needed = 0;
                GetPrinter(hPrinter, 5, IntPtr.Zero, 0, out needed);

                if (needed == 0)
                    throw new Win32Exception(Marshal.GetLastWin32Error(), "GetPrinter size failed");

                buffer = Marshal.AllocHGlobal((int)needed);

                if (!GetPrinter(hPrinter, 5, buffer, needed, out needed))
                    throw new Win32Exception(Marshal.GetLastWin32Error(), "GetPrinter failed");

                PRINTER_INFO_5 info =
                    (PRINTER_INFO_5)Marshal.PtrToStructure(buffer, typeof(PRINTER_INFO_5));

                // Microsoft Winspool attributes:
                // QUEUED            0x00000001
                // DIRECT            0x00000002
                ...
                //
                // Clear both QUEUED and DIRECT:
                // spool the job and print while it is still spooling.
                ...
                info.Attributes &= ~clearMask;
                Marshal.StructureToPtr(info, buffer, false);

                if (!SetPrinter(hPrinter, 5, buffer, 0))
                    throw new Win32Exception(Marshal.GetLastWin32Error(), "SetPrinter failed");
...
'@
    ...
    function Configure-RawCompletionMode {
        ...
        # Configure the queue through the native Windows Print Spooler API.
        # prncnfg.vbs is deliberately NOT used here: on the user's Windows 11
        # installation it prints Usage and exits 1 even for documented flags.
        ...
        try {
            Ensure-NativePrinterApi
            $attrs = [Lumitool.NativePrinter]::SetFastSpoolMode($PrinterName)
            UiLog ("QUEUE/WINSPOOL: SetPrinter level 5 OK, Attributes=0x" + $attrs.ToString("X8"))
        } catch {
            throw ("Windows Print Spooler API không cấu hình được queue: " + $_.Exception.Message)
        }

        # Verify using a separate WMI read so success is not assumed from
        # SetPrinter alone.
        $check = $null
        for ($i = 0; $i -lt 16; $i++) {
            ...
            if ($check -and
                (-not [bool]$check.Direct) -and
                (-not [bool]$check.Queued) -and
                (-not [bool]$check.EnableBIDI) -and
                (-not [bool]$check.KeepPrintedJobs) -and
                (-not [bool]$check.WorkOffline)) {
                break
            }
        }
        ...
        if ([bool]$check.Direct -or
            [bool]$check.Queued -or
            [bool]$check.EnableBIDI -or
            [bool]$check.KeepPrintedJobs -or
            [bool]$check.WorkOffline) {
            throw "Windows chưa áp dụng đúng Fast Spool Mode."
        }
    ...
    $form.Text = "Lumitool Printsever Setup V4.5"
    ...
    UiLog "Lumitool Printsever Setup V4.5"
    UiLog "Hỗ trợ mọi máy in có driver Windows đã được cài."
    UiLog "A=9101 · B=9102 · C=9103 · Fast Spool · Direct OFF · Watcher V2 auto-discovery"
