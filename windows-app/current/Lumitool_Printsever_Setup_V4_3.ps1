...
$LogPath = Join-Path $env:TEMP "Lumitool_Printsever_Setup_V4_3.log"
...
try {
    ...
    function Configure-RawCompletionMode {
        ...
        # Windows has three relevant states:
        #   Direct=True              -> send directly, no spooler buffering.
        #   Direct=False/Queued=True -> spool entire document before printing.
        #   Direct=False/Queued=False-> normal spooling, start printing immediately.
        #
        # Lumitool wants the third mode. It keeps the queue visible to Windows,
        # starts quickly, and lets Job Watcher V2 clear a stale Windows job as
        # soon as the ESP /api/jobs endpoint reports DONE.
        ...
        $scriptPath = Find-PrnCfgVbs
        if ([string]::IsNullOrWhiteSpace($scriptPath)) {
            throw "Không tìm thấy prncnfg.vbs của Windows để cấu hình spooler."
        }

        try {
            # Use Microsoft's supported printer configuration script instead
            # of Set-CimInstance. Some Windows 11 printer drivers return
            # 'Generic failure' when several Win32_Printer properties are
            # written through CIM even though prncnfg can update the queue.
            #
            # -direct : Spooling ON (do NOT print directly)
            # -queued : Start printing immediately; do not wait for full spool
            # -enablebidi/-keepprintedjobs/-workoffline/-enabledevq : OFF
            $args = @(
                "//NoLogo",
                $scriptPath,
                "-t",
            ...
            foreach ($line in @($output)) {
                if (-not [string]::IsNullOrWhiteSpace([string]$line)) {
                    UiLog ("QUEUE/PRNCNFG: " + [string]$line)
                }
            }

            if ($exit -ne 0) {
                throw ("prncnfg.vbs exit=" + $exit)
            }

            UiLog "QUEUE: Spooling=ON, StartImmediately=ON, Direct=OFF, BIDI=OFF"
        } catch {
            throw ("Windows không cấu hình được printer queue qua prncnfg: " + $_.Exception.Message)
        }

        # The spooler may need a moment to publish the changed attributes.
        $check = $null
        for ($i = 0; $i -lt 12; $i++) {
            ...
            if ($check -and
                ...
                (-not [bool]$check.KeepPrintedJobs)) {
                break
            }
        }
        ...
        if ([bool]$check.Direct -or [bool]$check.EnableBIDI -or [bool]$check.KeepPrintedJobs) {
            throw "Windows chưa áp dụng đúng Spooling/BIDI/KeepPrintedJobs."
        }

        if ([bool]$check.Queued) {
            UiLog "QUEUE: Windows đang ở chế độ đợi spool đủ tài liệu; chuyển lại Start immediately."
            try {
                $args2 = @("//NoLogo", $scriptPath, "-t", "-P", $PrinterName, "-queued")
                $output2 = & cscript.exe @args2 2>&1
                ...
                UiLog ("QUEUE: cảnh báo khi chuyển Start immediately: " + $_.Exception.Message)
            }
        }

        UiLog "QUEUE VERIFY: Spooling ON + Start printing immediately"
    }
    ...
    $form.Text = "Lumitool Printsever Setup V4.3"
    ...
    UiLog "Lumitool Printsever Setup V4.3"
    UiLog "Hỗ trợ mọi máy in có driver Windows đã được cài."
    UiLog "A=9101 · B=9102 · C=9103 · Spooling ON · Start immediately · Watcher V2 auto-discovery"
