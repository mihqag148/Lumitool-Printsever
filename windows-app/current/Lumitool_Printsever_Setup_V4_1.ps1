...
$LogPath = Join-Path $env:TEMP "Lumitool_Printsever_Setup_V4_1.log"
...
try {
    ...
    function Configure-RawCompletionMode {
        ...
        # Keep the printer as a normal Windows spooled queue so it appears
        # consistently in Printers & scanners and application print dialogs.
        # Job Watcher V2 handles stale Printing jobs after ESP confirms DONE.
        ...
        try {
            ...
            if (-not $wmiPrinter) { throw "Không tìm thấy Win32_Printer sau khi tạo queue." }

            Set-CimInstance -InputObject $wmiPrinter -Property @{
                EnableBIDI          = $false
            ...
            UiLog "QUEUE: Spooling=ON, Direct=OFF, BIDI=OFF, DevQuery=OFF"
        } catch {
        ...
        if (-not $check) { throw "Windows chưa đăng ký printer queue sau khi cấu hình." }
        ...
        if ([bool]$check.Direct -or -not [bool]$check.Queued -or [bool]$check.EnableBIDI -or [bool]$check.KeepPrintedJobs) {
            throw "Windows chưa áp dụng đúng cấu hình printer queue."
        }
    }
    ...
    $form.Text = "Lumitool Printsever Setup V4.1"
    ...
    UiLog "Lumitool Printsever Setup V4.1"
    UiLog "Hỗ trợ mọi máy in có driver Windows đã được cài."
    UiLog "A=9101 · B=9102 · C=9103 · Spooling ON · Watcher V2 auto-discovery"
