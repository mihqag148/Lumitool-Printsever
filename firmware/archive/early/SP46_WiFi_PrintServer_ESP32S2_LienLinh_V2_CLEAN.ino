...
// ===================== WIFI =====================
const char* WIFI_SSID = "Lien Linh";
const char* WIFI_PASS = "0986892367";
const char* HOST_NAME = "ESP32S2-Print-sever";
static const uint16_t PRINT_PORT = 9100;

// ===================== STATUS LED =====================
// On-board LED provided by user
static const uint8_t LED_PIN = 15;

// Change to true only if your board's LED works inverted.
static const bool LED_ACTIVE_LOW = false;

enum LedMode {
  LED_NORMAL_ON,        // normal: LED continuously ON
  LED_NO_NETWORK,       // cannot reach network/PC: slow blink
  LED_ERROR_PATTERN     // error: 2 quick blinks, pause, repeat
};

static LedMode ledMode = LED_NORMAL_ON;
static bool fatalUsbHostError = false;
static bool usbRuntimeError = false;
...
void updateStatusLed() {
  ...
  if (ledMode == LED_NO_NETWORK) {
    // Slow blink: 700 ms ON / 700 ms OFF
    bool on = ((now / 700UL) % 2UL) == 0;
    ledWrite(on);
    return;
  }

  // Error pattern:
  // ON 120ms -> OFF 120ms -> ON 120ms -> OFF 1200ms -> repeat
...
void refreshLedMode() {
  if (fatalUsbHostError || usbRuntimeError) {
    setLedMode(LED_ERROR_PATTERN);
  } else if (WiFi.status() != WL_CONNECTED) {
    setLedMode(LED_NO_NETWORK);
  } else {
    setLedMode(LED_NORMAL_ON);
  }
}
...
void setup() {
  ...
  ledWrite(true);   // LED on immediately when power is applied
