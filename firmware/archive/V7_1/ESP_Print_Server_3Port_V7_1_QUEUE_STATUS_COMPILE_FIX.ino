/*
  ESP PRINT SERVER V7.1 - 3 PRINTERS + PER-PRINTER JOB QUEUES
...
String renderPrinterCard(const PrinterSlot &p) {
  String s;
  s.reserve(2200);
  ...
  s += "<div class='small'>Queue: ";
  s += String(countAllJobs(p));
  s += "/";
  s += String(MAX_JOBS_PER_PRINTER);
  s += " · Receiving ";
  s += String(countJobs(p, JOB_RECEIVING));
  s += " · Waiting ";
  ...
  s += "<div class='small'>Done ";
  s += String(p.jobsDone);
  s += " · Failed ";
  s += String(p.jobsFailed);
  s += " · Printed ";
  s += formatBytes(p.bytesPrinted);
  s += "</div>";

  if (p.lastError != "None") {
    s += "<div class='bad small'>Last error: ";
...
String makePage() {
  ...
  p += "<h1>ESP Print Server ";
  p += macSuffix;
  p += "</h1>";

  p += "<div class='muted'>Firmware V7.1 · compile fix · 3 independent queues · Windows completion feedback · USB printer status</div>";

  p += "<div class='c'><h2>Mạng</h2><div class='grid'>";

  p += "<div class='kv'>Wi-Fi nhà<br><b>";
  p += WiFi.status() == WL_CONNECTED
         ? "Đã kết nối"
         : "Chưa kết nối";
  p += "</b></div>";
  ...
  p += "<div class='c'><h2>Máy in</h2>";
  p += renderPrinterCard(printers[0]);
  p += renderPrinterCard(printers[1]);
  p += renderPrinterCard(printers[2]);

  p += "<button onclick=\"location.href='/jobs'\">Xem hàng đợi & trạng thái job</button>";
  p += "</div>";

  p += "<div class='c'><h2>Bộ nhớ hàng đợi</h2>";
  p += "<div class='kv'>PSRAM: <b>";
  p += psramFound() ? "Có" : "Không";
  p += "</b><br>Đang cấp phát: <b>";
  p += formatBytes(totalQueueAllocated);
  p += "</b> / ";
  p += formatBytes(runtimeTotalQueueLimit());
  p += "<br>Giới hạn mỗi job: ";
